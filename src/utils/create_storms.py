...
class CreateStorms(object):
    ...
    def init(self, meta_dict, tide_dict):
        ...
        self.storm_dict = {}
        self.storm_dict["storm_file_type"] = (
            ".dat"  # note this is a placeholder. if the read_storms_h5 is implemented, this value needs to be moved to 1_file_creation.py
        )

        if (
            self.storm_dict["storm_file_type"] == ".h5"
        ):  # note: this feature hasn't been implemented yet; defaults to "elif" below
            ...
            for file in os.listdir(self.meta_dict["work_directory"]):
                if file.endswith(".dat"):
                    self.read_storms_dat(
                        file
                    )  # reading storms with file extension ".dat"
        return self.cshore_storms

    def read_storms_parquet(self, storm_file):
        """
        reading storm data from parquet file
        """
        ...
        for storm_id, storm_df in df.groupby("storm_id", sort=False):
            ...
            # Adding tides (optional: check if tide_dict is configured)
            if hasattr(self, "tide_dict") and self.tide_dict:
                # Re-using existing add_tides logic would require more refactoring
                # or exposing it. For now, keep core data structure.
                pass
    ...
    def read_storms_dat(self, storm_file):
        ...
        file_path = os.path.join(
            self.meta_dict["work_directory"], storm_file
        )  # path to file.
        ...
        date = [
            datetime.datetime.strptime(str(int(i)), "%Y%m%d%H%M") for i in date
        ]  # getting date/time in "python" format

        dt = self.dt_calc(
            date
        )  # calculating time step (assumes constant time step; seconds)
    ...
    def dt_calc(self, date):
        ...
        dt_mm = (
            date[1].minute - date[0].minute
        ) * 60  # min step (converted to seconds)
    ...
    def add_tides(self, storm_prfx, time, surge, hmo, hrms, tp):
        ...
        for i in self.tide_dict["phases"]:  # loop through tide phases
            for j, obj in enumerate(
                self.tide_dict["amp"]
            ):  # loop through tide amplitudes
                tide = obj * np.sin(
                    time_days * w + i * math.pi / 2 - w * t_max
                )  # idealized tide with amplitude and shift applied
                storm_name = (
                    storm_prfx + "_TPh" + str(i) + "_TAmp" + str(j + 1)
                )  # update storm name for tide specific contribution

                self.cshore_storms[storm_name] = {}  # saving all to dictionary
                self.cshore_storms[storm_name]["Hmo"] = hmo
                self.cshore_storms[storm_name]["Hrms"] = hrms
                self.cshore_storms[storm_name]["tp"] = tp
                self.cshore_storms[storm_name]["surge"] = (
                    surge + tide
                )  # adding the tide contribution to surge
                self.cshore_storms[storm_name]["time"] = time
