import os
import numpy as np
from utils.vfall import vfall_calc
...
class cshoreIO(object):
    ...
    def make_CSHORE_infile(self, fname, BC_dict, meta_dict, config):
        ...
        in_dict = {
            "header": [
                "------------------------------------------------------------",
                "Reach : %s  Profile : %s  Storm:  %s"
                % (meta_dict["Reach"], meta_dict["Profile"], meta_dict["Storm"]),
                "------------------------------------------------------------",
        ...
        in_dict["wf"] = vf.init(
            in_dict["d50"], in_dict["temp"], in_dict["salin"]
        )  # note: drs added this; fall velocity of sand grain
        in_dict["timebc_wave"] = BC_dict["timebc_wave"]
        in_dict["timebc_surg"] = in_dict[
            "timebc_wave"
        ...
        if in_dict["isedav"] == 1 and in_dict["iperm"] == 0 and in_dict["iveg"] == 0:
            fid.write(
                "%-8i                                  ->ICLAY  \n" % in_dict["iclay"]
            )  # this parameter is not specified in Brad's run_model...  This will crash if these conditions are met
        else:
            pass
        ...
        if in_dict["iperm"] == 1 or in_dict["isedav"] >= 1:
            for ii in range(0, len(BC_dict["x_p"])):
                fid.write(
                    "%11.4f%11.4f\n" % (BC_dict["x_p"][ii], BC_dict["zb_p"][ii])
                )  # this parameter is not specified in Brad's run_model...  This will crash if these conditions are met
        else:
            pass
    ...
    def read_CSHORE_infile(self, path):
        ...
        setup_bc = np.zeros(
            num_pts
        )  # the model must just assume this is zero, because this information IS NOT contained in the infile if ilab == 0
