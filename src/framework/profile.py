...
from .metrics import ProfileMetrics, fit_profile
...
def _shoreline_x(x: np.ndarray, z: np.ndarray, datum: float = 0.0) -> float:
    ...
    dry = (z > datum).astype(int)
    crossings = np.where(np.diff(dry) > 0)[0]
    if len(crossings) == 0:
        return float(x[0]) if np.all(z <= datum) else float(x[-1])
    ci = int(crossings[-1])
    dz = float(z[ci + 1] - z[ci])
    frac = (datum - float(z[ci])) / dz if abs(dz) > 1e-9 else 0.0
    return float(x[ci] + frac * (x[ci + 1] - x[ci]))
