...
_cshoreIO_cls = None   # lazy-loaded on first LocalCSHORERunner.__init__


def _load_cshoreIO():
    global _cshoreIO_cls
    if _cshoreIO_cls is None:
        # models/ lives under src/; add src/ to sys.path so it's importable
        # as a plain top-level package regardless of how the runner is invoked.
        _src = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
        if _src not in sys.path:
            sys.path.insert(0, _src)
        from models.cshore_io import cshoreIO
        _cshoreIO_cls = cshoreIO
    return _cshoreIO_cls
...
class LocalCSHORERunner(CSHORERunner):
    ...
    def run(self, profile: Profile, storm_forcing: dict) -> CSHOREResult:
        csio = _load_cshoreIO()()  # fresh instance per call → thread-safe (no class-level state)
