...
def run_interstorm(
    ...
    Each tick fires at the minimum of the two configured intervals.
    Profiles run in parallel threads; each tick is serialised across profiles
    before the next tick starts (no ordering dependency between profiles).
    """
    if t_storm <= t_start:
        return
    ...
    while t < t_storm:
        ...
        with ThreadPoolExecutor(max_workers=len(profiles)) as ex:
            futs = [
                ex.submit(_apply_tick, p, dt, t_tick, ecfg, slc)
                for p in profiles
            ]
            for f in futs:
                f.result()
