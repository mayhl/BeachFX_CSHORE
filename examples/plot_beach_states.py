...
def nourishment_profile(x, z):
    ...
    zn[berm_mask] = np.maximum(
        zn[berm_mask], np.interp(x[berm_mask], [350, 450, 700, 750], [9.5, 5.8, 5.8, 2.0])
    )
    return zn
...
ax.annotate(
    "", xy=(700, 0), xytext=(700, -1), arrowprops=dict(arrowstyle="->", color="#4a90d9", lw=1)
)
...
dz = z_storm - z_init
ax.fill_between(x, z_init, z_storm, where=dz < 0, color="#d62728", alpha=0.2, label="Erosion")
ax.fill_between(
    x, z_init, z_storm, where=dz > 0, color="#2ca02c", alpha=0.2, label="Accretion (bar)"
)
