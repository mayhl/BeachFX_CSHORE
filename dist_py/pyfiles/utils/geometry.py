...
class PopulateProfileSpace(object):
    ...
    def init(self, meta_dict, profile_dict, reach_num, reach):
        self.meta_dict = (
            meta_dict  # setting up self referenced values (sounds post-modern)
        )
        self.profile_dict = profile_dict

        for i in os.listdir(
            self.meta_dict["work_directory"]
        ):  # loop through work directory to find submerged profile
        ...
        if not self.submerged_file:  # else, raise error
            raise ValueError(
                "Submerged profile not found. Naming convention: 'ReachName_submerged_profile.txt"
            )

        self.setup_profile_max_min2(
            reach_num, reach
        )  # converting feet (from input) to meters (for cshore)
        self.create_profile_space2(
            reach_num, reach
        )  # compute profile space (variations between max and min profiles)
    ...
    def setup_profile_max_min2(self, reach_num, reach):
        ...
        """
        self.uw = self.profile_dict["width_upland"][reach_num] * 0.3048
        self.ue = self.profile_dict["height_upland"][reach_num] * 0.3048
        self.be = self.profile_dict["height_berm"][reach_num] * 0.3048

        self.dh = [i * 0.3048 for i in self.profile_dict["height_dune"][reach_num]]
        self.dw = [i * 0.3048 for i in self.profile_dict["width_dune"][reach_num]]
        self.bw = [i * 0.3048 for i in self.profile_dict["width_berm"][reach_num]]
    ...
    def create_profile_space2(self, reach_num, reach):
        ...
        submergedpath = os.path.join(
            self.meta_dict["work_directory"], self.submerged_file
        )
        ...
        for i_dh in self.dh:
            for i_dw in self.dw:
                for i_bw in self.bw:
                    i_dw_temp = int(round(i_dw / 0.3048))  # for file naming convention
                    i_bw_temp = int(round(i_bw / 0.3048))  # for file naming convention

                    profile_name = "{:0>4.1f}_{:0>3d}_{:0>3d}".format(
                        i_dh / 0.3048, i_dw_temp, i_bw_temp
                    )
                    ...
                    x_sub_temp = (
                        x_sub + x7
                    )  # shifting x_sub to start at end of the above profile
                    ...
                    new_x = np.concatenate(
                        (temp_x, x_sub_temp)
                    )  # appending things to form one x and z-array
                    new_z = np.concatenate((temp_z, z_sub))
                    new_x, new_z = self.reverse_l_r(new_x, new_z)  # flipping arrays
                    # print(new_x)
                    self.cshore_profiles["%s" % reach][profile_name]["x"] = (
                        new_x  # storing in dictionaries
                    )
                    self.cshore_profiles["%s" % reach][profile_name]["z"] = new_z
                    self.cshore_profiles["%s" % reach][profile_name]["d50"] = (
                        self.profile_dict["d50"][reach_num]
                    )
