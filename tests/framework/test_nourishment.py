...
from framework.nourishment import (
    ActiveCampaign, NourishmentConfig,
    _next_available, run_campaign,
)
...
def _p(pid: str = "p0", n: int = 50) -> Profile:
    ...
    return Profile(id=pid, x=x, zb=np.zeros(n), d50=0.3)


def _ncfg(volume_trigger: float = 0.001, production_rate: float = 500.0) -> NourishmentConfig:
    n = 50
