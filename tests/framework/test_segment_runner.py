...
from framework.interstorm import UniformErosionConfig
from framework.profile import Profile
from framework.reach import ReachContext, run_lifecycle
from framework.results import NullResultsSink, ParquetResultsSink
...
def _p(pid: str = "p0", n: int = 100) -> Profile:
    x = np.linspace(0, 200, n)
    return Profile(id=pid, x=x, zb=np.linspace(-2.0, 3.0, n), d50=0.3)
    ...
    for i in range(n):
        ...
        for dt_h in range(0, 13, 6):
            rows.append(dict(lifecycle=lifecycle, storm_id=f"S{i:02d}", hydro_tstp=dt_h,
                             ...
                             wave_height=1.5, wave_peak_period=10.0,
                             water_elevation=0.4, wave_direction=0.0))
    return pd.DataFrame(rows)


def _run(n_storms: int = 3, n_profiles: int = 2, cfg: ReachConfig | None = None,
         sink=None, lifecycle: int = 0) -> list[Profile]:
    profiles = [_p(f"p{i}") for i in range(n_profiles)]
    cfg = cfg or ReachConfig()
    sink = sink or NullResultsSink()
    ctx = ReachContext(reach_id="test", alternative_id="FWOP", results=sink,
                      sim_start=SIM_START, cfg=cfg)
    run_lifecycle(profiles, _storms(n_storms, lifecycle=lifecycle), SIM_START,
